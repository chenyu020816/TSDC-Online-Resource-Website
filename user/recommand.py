...
def course_suggest_list():
    try:
        ...
        keyword_list = [
            translate_keyword(keyword) for keyword in input_value["keyword"]
        ]
        print(keyword_list)
        best_n_id = get_best_n_resources(db, keyword_list, 2000, 5)
        print(best_n_id)
...
def search_resource():
    try:
        ...
        if user_id:
            resource_list = search_resource_by_id_list_with_rating_score(
                user_id, input_value["resource_list"]
            )
        else:
...
def give_rating():
    try:
        ...
        return_id = add_user_rating_history(
            db, input_value["user_id"], input_value["resource_id"], input_value["score"]
        )
...
def record_user_keyword():
    ...
    if search_history_id == -1:
        ...
        search_roadmap_field_ids = add_search_roadmap_fields_history(
            db, search_history_id, roadmap_domain
        )
        if search_roadmap_field_ids[0] == -1:
            ...
            for index, search_roadmap_field_id in enumerate(search_roadmap_field_ids):
                ...
                roadmap_fields_keyword_ids = add_roadmap_fields_keywords(
                    db, search_roadmap_field_id, keyword_list
                )
                best_n_id = get_best_n_resources(db, keyword_list, 2000, 5)
                search_resource_history_ids = add_search_resources_history(
                    db, search_roadmap_field_id, best_n_id.tolist()
                )
