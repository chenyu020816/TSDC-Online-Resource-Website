...
def crawl_hahow_resourse(keyword):
    url = "https://api.hahow.in/api/products/search?category=COURSE&limit=10&page=0&query={}&sort=RELEVANCE".format(
        keyword
    )
    ...
    if response.status_code == 200:
        ...
        for course in course_data["data"]["courseData"]["products"]:
            ...
            new_course = {
                ...
                "introduction": "",
                "url": (
                    f"https://hahow.in/courses/{course['uniquename']}"
                    if "uniquename" in course
                    else ""
                ),
                "image_url": (
                    course["coverImage"]["url"]
                    if "coverImage" in course and "url" in course["coverImage"]
                    else ""
                ),
                "source_platform": "Hahow",
                "resource_type": "pay course",
                "public_score": (
                    course["averageRating"] if "averageRating" in course else 0
                ),
                "user_score": 0,
                "num_of_purchases": (
                    course["numSoldTickets"] if "numSoldTickets" in course else 0
                ),
